...
class InsuranceModeling:
    ...
    def shap_analysis(self, model_name):
        ...
        explainer_premium = shape.Explainer(model_premium, self.X_train_premium)
        shap_values_premium = explainer_premium(self.X_test_premium)
        shape.summary_plot(shap_values_premium, self.X_test_premium)
        
        # SHAP analysis for TotalClaims
        explainer_claims = shape.Explainer(model_claims, self.X_train_claims)
        shap_values_claims = explainer_claims(self.X_test_claims)
        shape.summary_plot(shap_values_claims, self.X_test_claims)
